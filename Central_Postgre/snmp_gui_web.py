...
# Import du module d'envoi SNMP (partage avec Centrale_SQLite)
...
class SNMPMonitorApp:
    ...
    def get_anomalies_for_llm(self, limit=5):
        """Récupère les N dernières anomalies pour le LLM (PostgreSQL)"""
        if not self.db:
            return ""
        try:
            ...
            cursor.execute(
                ...
                "WHERE contenu_json LIKE %s "
                "ORDER BY timestamp_reception DESC LIMIT %s",
                ('%alerte_securite%', limit)
            )
            anomalies = []
                ...
                try:
                    ...
                    for niv in ('SUSPECT', 'ELEVEE', 'CRITIQUE'):
                        cursor.execute(
                            ...
                            "WHERE contenu_json LIKE %s "
                            "AND contenu_json LIKE %s",
                            ('%alerte_securite%', f'%"niveau": "{niv}"%')
                        )
                ...
                try:
                    ...
                    if niveau_filtre and niveau_filtre != "Tous":
                        cursor.execute(
                            ...
                            "WHERE contenu_json LIKE %s "
                            "AND contenu_json LIKE %s "
                            "ORDER BY timestamp_reception DESC LIMIT %s OFFSET %s",
                            ('%alerte_securite%', f'%"niveau": "{niveau_filtre}"%', PAGE_SIZE, offset)
                        )
                    else:
                        cursor.execute(
                            "SELECT * FROM paquets_recus "
                            "WHERE contenu_json LIKE %s "
                            "ORDER BY timestamp_reception DESC LIMIT %s OFFSET %s",
                            ('%alerte_securite%', PAGE_SIZE, offset)
                        )
                    for row in cursor.fetchall():
                        p = dict(row)
                        try:
