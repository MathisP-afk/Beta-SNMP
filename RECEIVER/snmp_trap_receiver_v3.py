#!/usr/bin/env python3
"""
SNMP v3 Collector v2.0 - Collecteur SNMPv3 Passif + Actif
Auteur: Étudiant 1 - Réseaux & Télécoms RT3
Projet: SAE 501-502 - Supervision Réseau
Description: Collecteur SNMPv3 (authPriv: SHA/DES) hybride.
             - PASSIF : Scapy capture TOUT le trafic SNMP du réseau
               (nécessite port mirroring SPAN sur le switch)
               Déchiffre les paquets SNMPv3 avec les credentials connus.
               Détecte GET, SET, GET_RESPONSE, TRAP, GET_NEXT, GET_BULK, etc.
             - ACTIF  : polling continu du switch via GET (complément)
             - Envoi immédiat à l'API REST

Dépendances:
    pip install pysnmp pycryptodomex cryptography requests scapy

Usage (PowerShell):
    .\\RECEIVER\\start_v3.ps1
"""
...
try:
    ...
    print(f"ERREUR: pysnmp non trouvé: {e}")
    print("Installez avec: pip install pysnmp pycryptodomex")
    sys.exit(1)

# HLAPI async pour le polling actif
try:
...
# Parseur BER manuel (remplace pyasn1 qui ne gère pas les tags context-specific)
PYASN1_AVAILABLE = True  # On utilise notre propre parseur BER

# Crypto pour déchiffrer DES
try:
...
# --- TYPES DE PDU SNMPv3 ---
# Tags ASN.1 context-specific pour les PDU SNMP
PDU_TYPE_TAGS = {
    0: 'GetRequest',
    1: 'GetNextRequest',
    2: 'Response',
...
def _password_to_key_sha(password: str, engine_id: bytes) -> bytes:
    ...
    1) Étend le mot de passe à 1 Mo par répétition → SHA1 → Ku
    2) Localise avec SHA1(Ku + engineID + Ku) → clé localisée
    """
    pwd = password.encode('utf-8')
    pwd_len = len(pwd)

    # Étape 1 : Ku = SHA1 de 1048576 octets du mot de passe répété
    hasher = hashlib.sha1()
    count = 0
    pwd_idx = 0
    while count < 1048576:
    ...
    # Étape 2 : Localisation avec l'Engine ID
...
def _decrypt_des_cbc(encrypted: bytes, priv_password: str,
    ...
    # IV = preIV XOR privacyParameters (salt)
...
def _ber_read_tlv(data: bytes, offset: int = 0):
    """Lit un TLV (Tag-Length-Value) BER à l'offset donné.
    Retourne (tag_byte, value_bytes, next_offset) ou None si fin.
    """
    if offset >= len(data):
        return None
    ...
    if offset + length > len(data):
        # Tronquer si nécessaire (padding DES)
...
def _ber_to_int(data: bytes) -> int:
    """Convertit des bytes BER INTEGER en int Python."""
    if not data:
        return 0
...
def _ber_to_oid(data: bytes) -> str:
    """Convertit des bytes BER OBJECT IDENTIFIER en string pointée."""
    if not data:
        return ''
    oid_parts = []
    first = data[0]
...
def _ber_value_to_str(tag_byte: int, value: bytes) -> str:
    ...
    elif tag_num == 0x40:  # Application types (Counter, Gauge, TimeTicks...)
        return str(_ber_to_int(value))
    else:
        # Types application SNMP (Counter32, Gauge32, TimeTicks, etc.)
        if tag_byte in (0x41, 0x42, 0x43, 0x46, 0x47):
            return str(int.from_bytes(value, 'big'))
        if tag_byte == 0x44:  # Opaque
            return value.hex()
        if tag_byte == 0x40:  # IpAddress
            if len(value) == 4:
...
# ============================================================================
# PARSEUR SNMPv3 BRUT (depuis paquets Scapy)
# ============================================================================

def parse_snmpv3_raw(raw_bytes: bytes, priv_password: str = None
                     ) -> Optional[Dict]:
    """Parse un paquet SNMPv3 brut (bytes UDP payload).
    Retourne un dict avec les métadonnées et, si possible, le PDU déchiffré.
    """
    try:
        # Message SNMPv3 = SEQUENCE { version, header, usm, data }
        ...
        # --- USM Security Parameters (OCTET STRING wrapping a SEQUENCE) ---
        ...
        # Le 4e composant : OCTET STRING si chiffré, SEQUENCE si en clair
        msg_data_tag = children[3][0]
        msg_data_value = children[3][1]

        if priv_flag and msg_data_tag == 0x04:
            # Données chiffrées (OCTET STRING)
            ...
            # Non chiffré (SEQUENCE = ScopedPDU)
            pdu_type, request_id, error_status, error_index, varbinds = \
...
def _parse_scoped_pdu_ber(raw_bytes: bytes,
                          already_unwrapped: bool = False) -> tuple:
    """Parse un ScopedPDU déchiffré ou en clair avec le parseur BER manuel.
    Retourne (pdu_type, request_id, error_status, error_index, varbinds)
    """
    pdu_type = 'Unknown'
    request_id = 0
    error_status = '0'
    ...
    try:
        if already_unwrapped:
            ...
            # raw_bytes = le déchiffré brut, commence par 0x30 (SEQUENCE)
        ...
        # Le PDU est le 3e composant (index 2)
        pdu_tag = scoped_children[2][0]
        pdu_value = scoped_children[2][1]

        # Détecter le type PDU depuis le tag
        # 0xA0=GET(0), 0xA1=GETNEXT(1), 0xA2=RESPONSE(2),
        # 0xA3=SET(3), 0xA5=GETBULK(5), 0xA6=INFORM(6),
        # 0xA7=TRAP(7), 0xA8=REPORT(8)
        tag_id = pdu_tag & 0x1F
...
class SNMPv3Collector:
    def __init__(self, listen_address: str, listen_port: int,
                 switch_ip: str, username: str,
                 auth_password: str, priv_password: str,
                 api_endpoint: str, api_key: str,
                 num_workers: int = 3, verbose: bool = True,
                 engine_id_hex: str = None,
                 interface: str = None,
        ...
        self.stats = {
            'sniffed': 0, 'decrypted': 0, 'polls': 0,
            'total_processed': 0,
            'api_sent': 0, 'api_failed': 0, 'alerts': 0
        }
        self.stats_lock = threading.Lock()

        logger.info("=" * 60)
        logger.info("[INIT] Collecteur SNMPv3 v2.0 (Passif + Actif)")
        logger.info(f"[INIT] Switch: {switch_ip}")
        logger.info(f"[INIT] Utilisateur SNMPv3: {username} (SHA/DES)")
        logger.info(f"[INIT] API: {api_endpoint}")
        logger.info(f"[INIT] Scapy (capture passive): "
                     f"{'OUI' if SCAPY_AVAILABLE else 'NON'}")
        logger.info(f"[INIT] Déchiffrement DES: "
                     f"{'OUI' if CRYPTO_AVAILABLE else 'NON'}")
        logger.info(f"[INIT] Polling actif (GET): "
    ...
    def analyser_severite(self, packet_info: Dict) -> str:
        ...
        # SET = toujours suspect (modification de config)
    ...
    def sniffer_thread(self):
        ...
        if not PYASN1_AVAILABLE:
            logger.error("[SNIFFER] Parseur BER non disponible")
            return

        logger.info("[SNIFFER] Démarrage capture passive Scapy...")
        ...
        def packet_callback(pkt):
            ...
            try:
                ...
                # Filtrer uniquement SNMP (ports 161/162)
                ...
                # Vérifier si c'est SNMPv3 (version = 3 dans le header)
                # Le premier byte d'un SEQUENCE est 0x30
                if raw_data[0] != 0x30:
                    return

                # Parser le paquet SNMPv3
                parsed = parse_snmpv3_raw(raw_data, self.priv_password)
                if not parsed:
                    return
                ...
                # Enrichir les varbinds avec les noms d'OID
                varbinds_enriched = []
                valeurs_dict = {}
                ...
                for vb in parsed.get('varbinds', []):
                    ...
                    if not oid_racine or oid_racine == "Non détecté":
                        oid_racine = oid_str

                # Construire le paquet pour l'API
                packet_info = {
                    'source_ip': src_ip,
                ...
                if severite in ['ELEVEE', 'CRITIQUE']:
                    packet_info['contenu']['alerte_securite'] = {
                        'niveau': severite,
                        'message': f"{pdu_type} suspect {src_ip}->{dst_ip}",
                        'timestamp': datetime.datetime.now().isoformat(),
                        'details': f"PDU: {pdu_type}, User: {parsed.get('username', '')}",
                ...
                if self.verbose:
                    decrypt_tag = "OK" if is_decrypted else "CHIFFRE"
                    vb_count = len(varbinds_enriched)
                    logger.info(
                        f"[SNIFFER] {src_ip}:{src_port} -> "
                        f"{dst_ip}:{dst_port} | "
                        f"{pdu_type} | {decrypt_tag} | "
                        f"{vb_count} OIDs | Sev: {severite}"
                    )

            except Exception as e:
                logger.debug(f"[SNIFFER] Erreur parsing paquet: {e}")

        # Lancer Scapy en mode sniff
        try:
            scapy_sniff(
        ...
        for packet in packets:
            try:
                payload = {
                    ...
                    'niveau_securite': packet.get('niveau_securite', 'authPriv'),
                ...
                logger.error(f"[API] Connexion impossible vers {url}")
            except Exception as e:
        ...
        while self.running:
            try:
                ...
                if packet:
                    ...
                    self.send_to_api([packet])
            except Exception as e:
                logger.error(f"[WORKER-{worker_id}] Erreur: {e}")

    # ----------------------------------------------------------------
    # POLLING ACTIF (async) - complément au sniffer passif
    # ----------------------------------------------------------------
    ...
    async def poll_switch(self):
        """Polling actif du switch par GET SNMPv3 à intervalle configuré."""
        if not HLAPI_AVAILABLE:
            logger.warning("[POLL] HLAPI non disponible, polling désactivé")
            return

        logger.info(f"[POLL] Polling actif vers {self.switch_ip} "
        ...
        while self.running:
            ...
            try:
                ...
                engine_id_hex = self.engine_id_hex or (
                    self.remote_engine_id.hex()
                    if self.remote_engine_id else ""
                )
    ...
    def stats_printer(self):
        while self.running:
            ...
            with self.stats_lock:
                s = self.stats
                logger.info(
                    f"[STATS] Sniffés:{s['sniffed']} | "
                    f"Déchiffrés:{s['decrypted']} | "
                    f"Polls:{s['polls']} | "
    ...
    async def start(self):
        # --- Engine ID ---
        if self.engine_id_hex:
            self.remote_engine_id = bytes.fromhex(self.engine_id_hex)
            logger.info(f"[START] Engine ID: 0x{self.engine_id_hex}")
        ...
        if SCAPY_AVAILABLE:
            ...
            sniffer_t.start()
            logger.info("[START] Sniffer passif Scapy lancé")
        else:
            logger.warning("[START] Scapy non disponible, pas de capture passive")

        # --- Polling actif (async task) ---
        poll_task = None
        if HLAPI_AVAILABLE:
            poll_task = asyncio.create_task(self.poll_switch())
            logger.info("[START] Polling actif lancé")

        logger.info("=" * 60)
        logger.info("[START] COLLECTEUR SNMPv3 OPÉRATIONNEL")
        logger.info(f"[START] Capture passive: {'OUI' if SCAPY_AVAILABLE else 'NON'}")
        logger.info(f"[START] Polling actif:   {'OUI' if HLAPI_AVAILABLE else 'NON'}")
    ...
    def print_final_stats(self):
        ...
        logger.info(f"  Paquets sniffés    : {s['sniffed']}")
        logger.info(f"  Paquets déchiffrés : {s['decrypted']}")
        logger.info(f"  Polls actifs       : {s['polls']}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Collecteur SNMPv3 passif + actif (SHA/DES)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Modes de capture:
  PASSIF: Scapy sniffe tout le trafic SNMP (nécessite SPAN sur le switch)
  ACTIF:  Polling GET continu vers le switch

Les deux modes tournent en parallèle.
        """
    )
    ...
    parser.add_argument('-p', '--port', type=int, default=162,
                        help="Port d'écoute UDP (défaut: 162)")
    parser.add_argument('-s', '--switch', required=True,
                        help='Adresse IP du switch')
    ...
    parser.add_argument('--priv-password', required=True,
    ...
    parser.add_argument('-e', '--engine-id', default=None,
                        help='Engine ID du switch en hexadécimal')
    parser.add_argument('-i', '--interface', default=None,
                        help='Interface réseau pour Scapy (ex: Ethernet)')
    parser.add_argument('--poll-interval', type=int, default=60,
                        help='Intervalle entre les polls actifs en secondes (défaut: 60)')
